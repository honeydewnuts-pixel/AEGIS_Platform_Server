  ...
  for E in [1.2,1.5]:
   for dr in [0,1]:
    ...
    for st in ['momentum','multiscale','break','exhaust']:
     ...
     c.append((L,E,'short' if dr==0 else 'long',st,(base&q)))
 def ev(m,s,e,dr):
...
# EURJPY
euj=pd.read_csv('/mnt/data/EURJPY_NEW12_M5.csv'); old=pd.read_csv('/mnt/data/v39_targets/EURJPY_M5_BID_ASK.csv'); old.datetime=pd.to_datetime(old.datetime.astype(str),format='%Y%m%d %H%M%S%f',errors='coerce'); euj.datetime=pd.to_datetime(euj.datetime,format='%Y%m%d %H%M',errors='coerce'); euj=pd.concat([old,euj],ignore_index=True).drop_duplicates('datetime').sort_values('datetime').reset_index(drop=True)
...
# GBPJPY
gbp=m1_2024_to_m5('/mnt/data/GBPJPY.zip'); old=pd.read_csv('/mnt/data/v39_targets/GBPJPY_M5_BID_ASK.csv'); old.datetime=pd.to_datetime(old.datetime.astype(str),format='%Y%m%d %H%M%S%f',errors='coerce'); gbp=pd.concat([old,gbp],ignore_index=True).drop_duplicates('datetime').sort_values('datetime').reset_index(drop=True)
