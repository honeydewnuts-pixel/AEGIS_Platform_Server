...
async def _bootstrap_admin_key() -> None:
    ...
    async with async_session_factory() as session:
        result = await session.execute(select(ApiKey).where(ApiKey.is_admin == True))  # noqa: E712
    ...
    if settings.ADMIN_BOOTSTRAP_KEY:
        ...
        async with async_session_factory() as session:
            ...
            session.add(ApiKeyModel(
                key_hash=hashlib.sha256(settings.ADMIN_BOOTSTRAP_KEY.encode()).hexdigest(),
                account_id=None,
                is_admin=True,
        ...
        raw_key = await issue_api_key(account_id=None, is_admin=True, label="auto-generated bootstrap admin key")
        logger.warning(
            "========================================\n"
