"""AEGIS Neural v2 inference layer.

The previous hand-coded logistic scorer has been replaced by a trained MLP
exported as plain JSON weights.  No PyTorch/TensorFlow runtime is required by
...
MODEL_PATH = Path(__file__).resolve().parent.parent / "models" / "aegis_neural_v2.json"
...
class NeuralAssistService:
    """Pure-Python inference for the trained AEGIS Neural v2 MLP."""

    def __init__(self) -> None:
        self.logger = configure_logging(__name__)
        self.model = self._load_model()
        if self.model.get("feature_dim") != FEATURE_DIM:
            raise ValueError(f"Neural model feature_dim mismatch: {self.model.get('feature_dim')} != {FEATURE_DIM}")
        self.logger.info("AEGIS Neural v2 loaded from %s", MODEL_PATH)
    ...
    def apply(self, history: list[dict[str, Any]], rule_result: dict[str, Any]) -> dict[str, Any]:
        ...
        out.update({
            ...
            "neural_model_version": self.model.get("version", "AEGIS_NEURAL_V2"),
        })
