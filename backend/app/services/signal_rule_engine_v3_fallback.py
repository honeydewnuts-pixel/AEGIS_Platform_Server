...
# SELF-CONTAINED BY DESIGN: these used to be imported from
# app.services.signal_rule_engine (the old v1/v2 module). In this
# checkpoint that module has been reduced to a 2-line compatibility
# shim that wildcard-re-exports the PRIMARY v3 engine's own RuleResult/
# crossed/side_of - which have a different shape (RuleResult here needs
# 7 required fields, not 4) and different semantics. Importing from
# there would silently bind this fallback's correctness to the very
# primary engine it exists to provide a safety net for - if primary
# breaks or its internals change again, the fallback could break
# alongside it, defeating the entire point of having one. Every
# primitive this file needs is defined locally instead, with no
# dependency on any other rule-engine module, live or otherwise.
...
def side_of(value_y: float, level_y: float, eps: float = 0.5) -> str:
    """Pixel-Y convention: smaller y = above (higher on screen/higher value)."""
    if value_y < level_y - eps:
        return "above"
    if value_y > level_y + eps:
        return "below"
    return "equal"
...
def crossed(prev_val: float, prev_level: float, cur_val: float, cur_level: float) -> str | None:
    prev_side = side_of(prev_val, prev_level)
    cur_side = side_of(cur_val, cur_level)
    if prev_side == cur_side or cur_side == "equal":
        return None
    return cur_side
...
class SignalRuleEngineV3Fallback:
    """
