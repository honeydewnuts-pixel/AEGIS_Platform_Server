...
def frame(p7u=100, p7m=150, p7l=200, p8u=110, p8m=150, p8l=190, rsi=150, ma=150):
    return {
        ...
        "band1": {"U": 80, "M": 140, "L": 200},
        "rsi6": rsi,
        "ma4": ma,
        "price_close": 210,
        "_indicator_top": 0,
        "_indicator_bottom": 300,
    }
...
def test_contraction():
    # p8 nested inside p7 in screen Y: p8U < p7U and p8L > p7L
...
def test_expansion_buy_rule():
    # expansion geometry + RSI above lower band in screen Y (rsi Y < L Y)
    h = [frame(p8u=120, p8l=180, rsi=50, ma=60), frame(p8u=120, p8l=180, rsi=50, ma=60)]
...
def test_expansion_sell_rule():
    # expansion geometry + RSI below upper band in screen Y (rsi Y > U Y)
